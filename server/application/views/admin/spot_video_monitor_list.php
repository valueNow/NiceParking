	...
	<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
		<form role="form" class="form-inline">
			<div class="form-group">
				...
				<input id="name" class="form-control" placeholder="视频监控名称" type="text">
			</div>
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/spot_video_monitor/index",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#name').val()!=''){
					d.name = $('#name').val();
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/spot_video_monitor/video_monitor_add" id="btnadd" class="btn btn-primary">添加视频监控</a>').insertBefore($('#editable_length'));
			}
		}
    } );
} );
