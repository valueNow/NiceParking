					...
					<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
						<form role="form" class="form-inline">
							<div class="form-group">
								<input type="text" placeholder="抢救装备名称" id="rescue_name" class="form-control">
							</div>
							<div class="form-group">
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/emergency_rescue/index",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				d.rescue_name = $('#rescue_name').val();
				if($('#type').val()!=''){
					d.type = $('#type').val();
				}
				if($('#eg_type').val()!=''){
		...
		"columnDefs": [
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$('<a href="/index.php/admin/emergency_rescue/add_rescue" id="btnadd" class="btn btn-primary ">新增抢救装备</a>').insertBefore($('#editable_length'));
			}
		}
    });
});
