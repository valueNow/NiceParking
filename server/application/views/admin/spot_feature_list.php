...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/spot_feature/index",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				d.feature_name = $('#feature_name').val();
            }
        },
		"order": [[ 1, "desc" ]],
		"columnDefs": [
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$('<a href="/index.php/admin/spot_feature/add_spot_feature" id="btnadd" class="btn btn-primary ">新增特色旅游点</a>').insertBefore($('#editable_length'));
			}
		}
    });
});
