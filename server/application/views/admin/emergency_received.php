	...
	<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
		<form role="form" class="form-inline">
			<div class="form-group">
				...
				<input id="rec_name" class="form-control" placeholder="信息名称" type="text">
			</div>
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/emergency/received",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#rec_name').val()!=''){
					d.rec_name = $('#rec_name').val();
				}
            }
        },
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/emergency/add_received" id="btnadd" class="btn btn-primary">添加接报信息</a>').insertBefore($('#editable_length'));
			}
		}
    } );
} );
