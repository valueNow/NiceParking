...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/emergency_safeguard/index",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#eg_type').val()!=''){
		...
		"columnDefs": [
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$('<a href="/index.php/admin/emergency_safeguard/add_safeguard" id="btnadd" class="btn btn-primary ">新增应急保障计划</a>').insertBefore($('#editable_length'));
			}
		}
    });
});
