	...
	<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
		<form role="form" class="form-inline">
			<div class="form-group">
				...
				<input id="event_name" class="form-control" placeholder="事件名称" type="text">
			</div>
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/emergency/event",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#event_name').val()!=''){
					d.event_name = $('#event_name').val();
				}
            }
        },
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/emergency/add_event" id="btnadd" class="btn btn-primary">事件添加</a>').insertBefore($('#editable_length'));
			}
		}
    } );
} );
