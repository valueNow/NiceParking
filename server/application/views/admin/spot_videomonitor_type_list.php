...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/spot_video_monitor/stype_index",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#type_name').val()!=''){
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/spot_video_monitor/stype_add" id="btnadd" class="btn btn-primary">添加类型</a>').insertBefore($('#editable_length'));
			}
		}
    } );
} );
