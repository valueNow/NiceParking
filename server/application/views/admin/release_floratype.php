	...
	<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
		<form role="form" class="form-inline">
			<div class="form-group">
				...
				<input id="name" class="form-control" placeholder="分类名称" type="text">
			</div>
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/release/floratype",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#name').val()!=''){
					d.name = $('#name').val();
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/release/add_floratype" id="btnadd" class="btn btn-primary">添加分类</a>').insertBefore($('#editable_length'));
			}
			if($('#btnstatuelst').length<=0){
				$('<div class="col-sm-12" id="btnstatuelst"><div class="row" style="padding-left:15px"><input type="submit" value="排序" ></div></div>').insertBefore($('#editable_info').parent());
			}
		}
    } );
} );
