	...
	<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
		<form role="form" class="form-inline">
			<div class="form-group">
				...
				<input id="duty_name" class="form-control" placeholder="值班人姓名" type="text">
			</div>
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/emergency/duty",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#duty_name').val()!=''){
					d.duty_name = $('#duty_name').val();
				}
            }
        },
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/emergency/add_duty" id="btnadd" class="btn btn-primary">添加值班信息</a>').insertBefore($('#editable_length'));
			}
		}
    } );
} );
