	...
	<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
		<form role="form" class="form-inline">
			<div class="form-group">
				...
				<input id="interest_name" class="form-control" placeholder="兴趣点名称" type="text">
			</div>
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/spot_interest/interest",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				if($('#interest_name').val()!=''){
					d.interest_name = $('#interest_name').val();
				}
            }
        },
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$(' <a href="/index.php/admin/spot_interest/interest_add" id="btnadd" class="btn btn-primary">添加兴趣点</a>').insertBefore($('#editable_length'));
			}
		}
    } );
} );
