					...
					<div class="ibox-content" style="padding-top:15px !important;padding-bottom:15px !important;"  id="quary">
						<form role="form" class="form-inline">
							<div class="form-group">
								<input type="text" placeholder="仓库名称" id="storehouse_name" class="form-control">
							</div>
							<div class="form-group">
...
$(document).ready(function() {
   table= $('#editable').DataTable( {
        ...
        "ajax": {
            "url": "/index.php/admin/emergency_material_storehouse/index",
			"data": function ( d ) {  
                //添加额外的参数传给服务器
				d.storehouse_name = $('#storehouse_name').val();
				if($('#status').val()!=''){
					d.status = $('#status').val();
				}
            }
		...
		"columnDefs": [
		 ...
		 "drawCallback": function( settings ) {
			...
			if($('#btnadd').length<=0){
				$('<a href="/index.php/admin/emergency_material_storehouse/add_storehouse" id="btnadd" class="btn btn-primary ">新增物资仓库</a>').insertBefore($('#editable_length'));
				//$('<a href="#" onclick="window.parent.fnAddModelInMap(\'/index.php/admin/emergency_material_storehouse/add_storehouse\',\'POI\',\'/statics/image/poi/ATM.png\');" id="btnadd" class="btn btn-primary ">新增物资仓库</a>').insertBefore($('#editable_length'));
			}
		}
    });
});
