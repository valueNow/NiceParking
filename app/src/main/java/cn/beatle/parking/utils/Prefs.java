...
public class Prefs {
	 ...
	 *         ClassCastException if there is a preference with this name that
	 *         is not a Set.
	 * @see android.content.SharedPreferences#getStringSet(String,
	 *      java.util.Set)
	 */
	/*
	 * @TargetApi(Build.VERSION_CODES.HONEYCOMB) public static Set<String>
	 * getStringSet(final String key, final Set<String> defValue) {
	 * SharedPreferences prefs = getPreferences(); if (Build.VERSION.SDK_INT >=
	 * Build.VERSION_CODES.HONEYCOMB) { return prefs.getStringSet(key,
	 * defValue); } else { if (prefs.contains(key + "#LENGTH")) {
	 * HashSet<String> set = new HashSet<String>(); // Workaround for pre-HC's
	 * lack of StringSets int stringSetLength = prefs.getInt(key + "#LENGTH",
	 * -1); if (stringSetLength >= 0) { for (int i = 0; i < stringSetLength;
	 * i++) { prefs.getString(key + "[" + i + "]", null); } } return set; } }
	 * return defValue; }
	 */
	 ...
	 * @param value
	 *            The new value for the preference.
	 * @see android.content.SharedPreferences.Editor#putStringSet(String,
	 *      java.util.Set)
	 */
	/*
	 * @TargetApi(Build.VERSION_CODES.HONEYCOMB) public static void
	 * putStringSet(final String key, final Set<String> value) { final Editor
	 * editor = getPreferences().edit(); if (Build.VERSION.SDK_INT >=
	 * Build.VERSION_CODES.HONEYCOMB) { editor.putStringSet(key, value); } else
	 * { // Workaround for pre-HC's lack of StringSets int stringSetLength = 0;
	 * if (mPrefs.contains(key + "#LENGTH")) { // First read what the value was
	 * stringSetLength = mPrefs.getInt(key + "#LENGTH", -1); } editor.putInt(key
	 * + "#LENGTH", value.size()); int i = 0; for (String aValue : value) {
	 * editor.putString(key + "[" + i + "]", aValue); i++; } for (; i <
	 * stringSetLength; i++) { // Remove any remaining values editor.remove(key
	 * + "[" + i + "]"); } } if (Build.VERSION.SDK_INT <
	 * Build.VERSION_CODES.GINGERBREAD) { editor.commit(); } else {
	 * editor.apply(); } }
	 */
